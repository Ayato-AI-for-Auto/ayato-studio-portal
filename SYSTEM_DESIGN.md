...
### 2.1. レスポンシブデザインの基本原則
**「Tailwind CSS のメディアクエリ（sm:, md:, lg:等）を駆使して、単一のコードベースのままデバイスの【画面幅】に応じてUIがシームレスに変形するレスポンシブデザインを極める」**

*   **単一ソース・オブ・トゥルース (SSOT)**: 
    *   スマートフォン用とPC用でコンポーネントやレイアウトを分割（二重管理）することは**固く禁ずる**。
    *   デバイス（User-Agent等）による条件分岐は行わず、常に単一のReactコンポーネントが全画面サイズに対応する。
*   **モバイルファースト戦略**:
    *   Tailwindのユーティリティクラス（プレフィックスなし）を用いて、最小画面幅（スマートフォン）のレイアウトをデフォルトとして構築する。
    *   より大きな画面（タブレット、PC）に対しては、適宜 `md:`, `lg:` などのブレイクポイントプレフィックスを使用して、余白、フォントサイズ、レイアウト構造（Flex, Gridの切り替え等）を上書きしていく。
*   **SSGとの互換性確保**:
    *   現在採用している完全静的サイト生成 (`output: 'export'`) との互換性を完全に保つため。
    *   サーバーサイドでのレンダリング結果とクライアントサイドでのレンダリング結果が一致することを保証し、Hydration Mismatch を物理的に排除する。
